<h1 align="center">
    ...
    &nbsp;
    <a href="https://doc.traefik.io/traefik/"><img src="assets/traefik.svg" alt="Traefik Proxy" width="100"></a>
    <br>
        Laravel Dockerized
    <br>
</h1>
...
- [Run Containers](#run-containers)
  ...
  - [Run Queue Container](#run-queue-container)
  - [Run Scheduler Container](#run-scheduler-container)
...
_`amqp`_ _`mongodb`_ _`ds`_ _`igbinary`_ _`newrelic`_ _`msgpack`_ _`redis`_

#### [Zend Modules]

_`Xdebug`_ _`OPcache`_
...
- **Open `.env` file and edit `PROJECT_NAME` and `DOMAIN` variables**
    - The value of the `DOMAIN` variable from the [*scripts/envs/deploy.env*](scripts/envs/deploy.env#L1) file must match the same value as the same variable from the `.env` file
- **Search for `yourdomain.tld` within that same Docker folder and replace with your company's domain**
- Copy [*scripts/envs/docker.env*](scripts/envs/docker.env) file to root `docker` folder
...
1. Copy the contents of the [*services/app/.env.compose*](services/app/.env.compose) file and place at the end of the `.env` file
...
4. Download the _Laravel_ application repository. The `APP_LOCAL_FOLDER` variable in the `.env` file must have the same name as the Laravel application folder
...
1. Copy the contents of the [*services/nginx/.env.compose*](services/nginx/.env.compose) file and place at the end of the `.env` file
...
1. Copy the [*scripts/envs/renew.env*](scripts/envs/renew.env) file to the docker root folder(same value as the `DOCKER_PATH` variable in the `.env` file)
...
After, update the variable `WEBSERVER_CERTBOT_IMAGE` with the name of the newly created image, **then run the script [*./scripts/renew-certs.sh*](scripts/renew-certs.sh) and follow the steps to create the first certificates for the application**.
...
- The `traefik` network name, must be updated in the [*traefik/traefik.yml*](traefik/traefik.yml) file in the `providers.docker.network` directive. Replacing `company_traefik_network` with the value `${VARIABLE_VALUE_PROJECT_NAME}_traefik_network`
...
- Copy [*scripts/envs/deploy.env*](scripts/envs/deploy.env) file to docker root folder
...
- Edit the [*services/app/.env.container*](services/app/.env.container) file, setting the variables for the laravel project, mainly `APP_KEY` and `APP_ENV`, which are mandatory in the `entrypoint` docker of the application container
...
- See if the domain is correct in the [`server_name`](services/nginx/servers/app.conf#L27) directive in the [*services/nginx/servers/app.conf*](services/nginx/servers/app.conf) file

- **Run the [*scripts/deploy-version.sh*](scripts/deploy-version.sh) script to create the Laravel/PHP-FPM and Webserver/NGINX containers**
...
- Edit the passwords in the [*services/authelia/secrets*](services/authelia/secrets) folder
    ...
    - [`redis`](https://www.authelia.com/docs/configuration/session/redis.html): This is a session provider. Must be the same as the `requirepass` directive in the file [*services/redis/redis.conf*](services/redis/redis.conf#L885)
...
### Run [Queue](php/docker-entrypoint.sh#L302) Container

- Open [*`.env.container`*](services/queue/.env.container) file and edit as desired:
    ...
    - **`LARAVEL_QUEUE_MANAGER`**: This environment variable defines the container context, and can have the following values:
        - **worker**: Configure the supervisor to run many processes in the Laravel command `artisan queue:work`
            - Many processes running in the Laravel artisan [`queue:work`](php/queue/templates/laravel-worker.conf.tpl) for queue management
...
### Run [Scheduler](php/docker-entrypoint.sh#L368) Container

- Open [*`.env.container`*](services/scheduler/.env.container) file and edit as desired:
...
- Copy the contents of the [*services/goaccess/.env.compose*](services/goaccess/.env.compose) file and place at the end of the `.env` file
...
### [`./scripts/cron-renew-certs.sh`](scripts/cron-renew-certs.sh)
...
- `-add`: *Add a command that will execute [./scripts/renew-certs.sh](scripts/renew-certs.sh) to CRON which will be executed every time set in the `--timer` option*
...
### [`./scripts/deploy-version.sh`](scripts/deploy-version.sh)
...
### [`./scripts/loadbalancer-nginx.sh`](scripts/loadbalancer-nginx.sh)
...
- `--php-container-name=`: Option used so that Docker can filter PHP/Laravel containers with the command `docker ps --filter name="$PHP_CONTAINER_NAME"`
    ...
    - This option is only needed when containers are not generated by the [*./scripts/deploy-version.sh*](scripts/deploy-version.sh) script or `docker-up-app` command
...
- `--nginx-container-name=`: Option used so that Docker can filter NGINX/Webserver containers with the command `docker ps --filter name="$NGINX_CONTAINER_NAME"`
    ...
    - This option is only needed when containers are not generated by the [*./scripts/deploy-version.sh*](scripts/deploy-version.sh) script or `docker-up-webserver` command
...
### [`./scripts/update-app.sh`](scripts/update-app.sh)
...
- Run the [**`./scripts/renew-certs.sh`**](scripts/renew-certs.sh) script and follow the steps of *certbot* certificate generation
